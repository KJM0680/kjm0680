...
create table if not exists public.equipment_history (id bigint generated by default as identity primary key, equipment_id text not null, vibration numeric not null, temperature numeric not null, recorded_at timestamptz not null default now());

do $$ declare t text; begin
  foreach t in array array['equipment','alerts','inspections','materials','spares','spare_monitor','equipment_history'] loop
...
insert into public.equipment_history (equipment_id,vibration,temperature) select id,vibration,temperature from public.equipment where not exists (select 1 from public.equipment_history h where h.equipment_id=equipment.id);
